...
process.env.VITE_APP_TITLE ??= process.env.OVERVIEW_TITLE ?? 'UrbanPermit';
process.env.VITE_APP_DESCRIPTION ??= process.env.OVERVIEW_DESCRIPTION ?? 'UrbanPermit - Votre assistant urbanisme';
...
// Dynamically load optional platform plugins
async function loadOptionalPlugins() {
  ...
  try {
    ...
    // Plugin not available (e.g., on Vercel), skip it
  }
  try {
